...
public interface DailyTrackRepository extends CrudRepository<DailyRecord,String> {
    ...
    @Query(value = "SELECT user_id, timestamp, device_id FROM user_routes_daily WHERE user_id = :userNumber", nativeQuery = true)
     List<DailyRecord> getUserRoute(@Param("userNumber")String phoneNumber);
    ...
    List<DailyRecord> getUserRouteFromParticularTime(@Param("userNumber")String phoneNumber, @Param("start")long startDate, @Param("end")long endDate);
